...
import { saveMessages, loadMessages, deleteSession } from "./session-persistence.js";
...
export function createAgentSessionDOClass<Env extends AgentEnv>(
  ...
  return class AgentSessionDO implements DurableObject {
    ...
    async webSocketMessage(ws: WebSocket, raw: string | ArrayBuffer): Promise<void> {
      ...
      if (!msg) {
        this._sendTo(ws, { type: "error", message: "Invalid message format", code: "PARSE_ERROR" });
        return;
      }
      ...
      try {
        ...
        this._sendTo(ws, { type: "error", message: e.message ?? "Internal error", code: "INTERNAL" });
      }
    }
